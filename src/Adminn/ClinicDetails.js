...
const ClinicDetails = () => {
  ...
  useEffect(() => {
    const fetchClinicDetails = async () => {
      try {
        const response = await fetch(`https://localhost:7190/api/Clinic/GetClinicById?clinicId=${clinicId}`);
        if (response.ok) {
    ...
    const fetchClinicDoctors = async () => {
      try {
        const response = await fetch(`https://localhost:7190/api/Doctor/clinic/${clinicId}`);
        if (response.ok) {
          const data = await response.json();
