...
const EditDoctor = ({ match }) => {
    ...
    const fetchExistingDoctor = async () => {
      try {
        const response = await fetch(
          `https://localhost:7207/api/Doctor/GetDoctorById?doctorId=${id}`
        );
        if (response.ok) {
          ...
          console.error(
            'Failed to fetch existing doctor:',
            response.statusText
          );
        }
      } catch (error) {
  ...
  const handleUpdateDoctor = async () => {
    try {
      ...
      const response = await fetch(
        `https://localhost:7207/api/Doctor/UpdateDoctor/${id}`,
        {
          method: 'PUT',
          headers: {
      ...
      <form>
        ...
        <div className='form-group'>
          ...
          <input
            ...
            id='personalNumber'
            name='personalNumber'
            value={existingDoctor.personalNumber}
            onChange={handleInputChange}
          />
        </div>
        <div className='form-group'>
        ...
        <div className='form-group'>
          ...
          <input
            ...
            id='phoneNumber'
            name='phoneNumber'
            value={existingDoctor.phoneNumber}
            onChange={handleInputChange}
          />
        </div>
        <div className='form-group'>
          ...
          <input
            ...
            id='dateOfBirth'
            name='dateOfBirth'
            value={existingDoctor.dateOfBirth}
            onChange={handleInputChange}
          />
        </div>
        <div className='form-group'>
          ...
          <input
            ...
            id='specialisation'
            name='specialisation'
            value={existingDoctor.specialisation}
            onChange={handleInputChange}
          />
        </div>
        <div className='form-group'>
          ...
          <div className='btn-group' role='group'>
            ...
            <input
              ...
              value='female'
              onChange={handleInputChange}
              checked={existingDoctor.gender === 'female'}
              className='btn-check'
              required
            />
