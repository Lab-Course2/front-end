...
const Navbar = ({ isLoggedIn, handleLogout,signalR}) => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const fetchData = await fetch(`https://localhost:7190/api/Notification/Notificatoin/user?id=${userId}`, {
        method: 'GET',
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response =  await fetch(`https://localhost:7190/api/Notification/Notificatoin/delete?${queryParams}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        }
  ...
  const handleMarkAsRead = async (id) => {
    try {
      ...
      const response = await fetch(`https://localhost:7190/api/Notification/Notificatoin/update?id=${index.idNotification}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
