...
function MyPendingAppointments() {
                ...
                if (response.ok) {
                    ...
                    const appointmentDetailsPromises = appointmentData
                        ...
                        .map(async appointment => {
                            const response = await fetch(`https://localhost:7190/api/AppointmentSlot/${appointment.appointmentSlotId}`);
                            if (response.ok) {
                                const slotData = await response.json();
                                const doctorResponse = await fetch(`https://localhost:7190/api/Doctor/GetDoctorById?doctorId=${slotData.doctorId}`);
    ...
    const cancelAppointment = async (id) => {
        try {
            const response = await fetch(`https://localhost:7190/api/Appointment/PatientCancelAppointmentPostMethod?id=${id}`, {
                method: 'POST',
                headers: {
