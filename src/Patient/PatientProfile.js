...
export default function ProfilePage({signalRHub}) {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await fetch(`https://localhost:7190/api/Patient/GetPatientById/?patientId=${userId}`);
        if (!response.ok) {
    ...
    const CheckForFriendRequest = async () => {
      try {
          ...
          const response = await fetch(`https://localhost:7190/api/RequestConnection/CheckConnection?userId=${loginId}&touserId=${userId}`, {
              method: 'GET',
              headers: {
  ...
  const handleRejectConnection = async()=>{
    try {
      const fetchData = await fetch(`https://localhost:7190/api/RequestConnection/CancelRequestConnection?idRequest=${idRequest}`,{
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json'
        }
    ...
    switch (connectionType) {
      case 'Accepted':
        const RemoveFromFriendList = async () =>{
          const fetchData = await fetch(`https://localhost:7190/api/Connections/DeleteFromFriends?userId=${localStorage.getItem('userId')}&friendId=${userId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            },
      ...
      case 'Waiting':
       ...
       const AcceptUser = async()=>{
            try {
              const fetchData = await fetch(`https://localhost:7190/api/RequestConnection/AcceptRequest?Id=${idRequest}`, {
                  method: 'POST',
                  headers: {
                      'Content-Type': 'application/json'
                  },
        ...
        const AddToList = async () => {
          try {
            ...
            const formattedDateTime = currentDate.toISOString().slice(0, -1);
              ...
              const fetchData = await fetch(`https://localhost:7190/api/RequestConnection/AddConnection`, {
                  method: 'POST',
                  headers: {
                      'Content-Type': 'application/json'
                  },
